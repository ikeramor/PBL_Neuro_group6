...
Alzheimer’s Disease (AD) is the leading cause of dementia worldwide, and its diagnosis in the early stages remains a clinical challenge due to the symptomatic overlap with normal aging. In response, neuroimaging provides a non-invasive approach capable of capturing structural and functional alterations in the brain associated with this disease. This project investigates myelin-functional coupling within Resting-State Networks (RSN) as a potential biomarker for Deep-Learning (DL) based classification and diagnosis of AD. Structural information is derived from T1-weighted and T2-weighted Magnetic Resonance Imaging (sMRI) scans, from which myelin-sensitive proxies are computed. Functional information, in turn, is derived from resting-state functional Magnetic Resonance Imaging (rs-fMRI) scans through the characterization of Functional Connectivity (FC) patterns. Results showed widespread myelin reduction in AD, independent of normal aging, across all seven Yeo RSNs. RSFC analyses revealed predominantly reduced connectivity in AD compared with Cognitively Normal (CN) individuals, accompanied by significantly lower myelin-functional coupling across all Yeo networks. The best classification performance was achieved by a 3D-CNN trained on T2-weighted MRI volumes (macro-average F1-score: 80.75%). LSTM-based analyses also identified structured temporal patterns in RSN dynamics, although extracting AD-specific features remained challenging.
